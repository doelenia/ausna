...
export function CommunitiesTab() {
  ...
  const loadPortfolios = async (currentPage: number = 1) => {
    ...
    setError(null)
    try {
      const result = await searchPortfolios('community', searchQuery, currentPage, 10)
      if (result.success) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-4">
        <input
          ...
          placeholder="Search by creator name, community name, or ID..."
      ...
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-4">
          Communities {total > 0 && `(${total} total)`}
        </h2>
        {loading ? (
          <div className="text-gray-500">Loading communities...</div>
        ) : portfolios.length === 0 ? (
          <div className="text-gray-500">
            {searchQuery ? 'No communities found. Try a different search term.' : 'No communities found.'}
          </div>
        ) : (
          <>
                  ...
                  {portfolios.map((portfolio) => (
                    <tr key={portfolio.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                        <Link
                          href={`/portfolio/community/${portfolio.id}`}
