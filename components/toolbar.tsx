...
export const Toolbar = ({
	...
	const onInput = (value: string) => {
		setValue(value);
		update({ id: initialData._id, title: value || "Untitled Page" });
	};
	...
	return (
		<div className="pl-[54px] group relative">
			{!!initialData && !preview && (
				<div className="flex items-center gap-x-2 group/icon pt-6">
					<IconPicker onChange={onIconSelect}>
						<p className="text-[80px] hover:opacity-75 transition">
							{initialData.icon}
						</p>
					</IconPicker>
					<Button
			...
			{!!initialData.icon && preview && (
				<p className="text-[80px] pt-6">
					{initialData.icon}
				</p>	
			)}
			<div className="opacity-0 group-hover:opacity-100 flex items-center gap-x-1 py-0">
				{!initialData.icon && !preview && (
