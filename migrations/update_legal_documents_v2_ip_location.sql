-- Legal documents v2: add disclosure that we may derive city/region from IP by default.
-- Run this after create_legal_documents_tables.sql (or when v1 is already in place).

-- Deactivate current active versions
UPDATE legal_documents
SET is_active = false
WHERE type IN ('terms', 'privacy') AND is_active = true;

-- Insert Terms & Conditions version 2 (adds IP/location disclosure in section 9)
...
9. Privacy
Your use of the Service is also governed by our Privacy Policy, which explains how we collect, use, and protect your personal information. By using the Service, you acknowledge that you have read and understood the Privacy Policy. We may derive approximate location (such as city or region) from your IP address by default, as described in the Privacy Policy.
...
-- Insert Privacy Policy version 2 (adds section 2.6 Location derived from IP)
INSERT INTO legal_documents (type, version, content, effective_date, is_active)
VALUES (
  'privacy',
  2,
...
2.6 Location derived from IP address
We may derive approximate geographic location (such as city, region, or country) from your IP address by default. This helps us with security, analytics, and improving the Service. The derived location is not your precise physical address and may be inaccurate (for example when using VPNs or certain network configurations).
