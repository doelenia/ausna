...
export default async function SpaceIndexPage({ searchParams }: SpaceIndexPageProps) {
  const supabase = await createClient()

  const limit = searchParams.limit ? parseInt(searchParams.limit) : 20
  const offset = searchParams.offset ? parseInt(searchParams.offset) : 0

  let query = supabase.from('portfolios').select('*').neq('type', 'human')

  if (searchParams.type) {
    const types = Array.isArray(searchParams.type) ? searchParams.type : [searchParams.type]
    const allowed = types.filter((t) => DB_NON_HUMAN_TYPES.includes(t))
    if (allowed.length > 0) {
  ...
  return (
    <div className="mb-8">
      ...
      <div className="flex gap-2 mb-4">
        <Button asLink href="/human" variant="secondary">
          <UIText>Humans</UIText>
        </Button>
        <Button asLink href="/space" variant="primary">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {portfolios && portfolios.length > 0 ? (
          ...
          <div className="col-span-full text-center py-12">
            <Content>No spaces found.</Content>
          </div>
        )}
      </div>
    </div>
